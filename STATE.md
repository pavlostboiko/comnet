...
- Прод — **новий сервер `jawa`** (Debian 13, системний nginx-проксі, **HTTPS**; сабдомен). Деплой/ре-імпорт виконує КОРИСТУВАЧ (**ssh з агента не працює**). Деплой оновлень: `git pull` → `docker compose up -d --build` (міграції самі).
