...
**Оновлено:** 2026-07-30 · **HEAD:** `f9c1b20` (гілка `main`, запушено).
Ядро v2 у `main`, **v1 повністю знесено** (міграції 012-**026**). **Кожна зміна — через гілку → `merge --no-ff` у `main` → push.**
...
- Керування: Довідники → МВО (форма «Тип», пошуковий пікер особи, **посада/звання на записі МВО**, повне **редагування** особи/посади/звання/дат + **видалення**).
...
- **Довідники** (`structure`): вкладки Служби/Підрозділи/Склади/МВО/Групи/Особи + **Типи операцій** і **Реквізити** (перенесені зі старого /settings; бек `/api/settings`). МВО: Тип (Склад/Фінслужба загальна), пошуковий пікер, ✎/✕.
...
**1** Користувачі-edit · **2** Служби-edit · **7** історія видач стійка до видалення особи (assignment.person_name snapshot + FK SET NULL + заборона видалення з активною видачею; міграція) · **9** № картки — текст? · **10** пошуковий дропдаун осіб (сорт + «Прізвище Імʼя (Позивний)») усюди · **11** редагувати категорії · **12** список рухів у «Переміщення» · **13** прибрати мертвий код `op_type_details`.
