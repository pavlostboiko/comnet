...
class PrintDocument(Base):
    ...
    items = relationship("PrintDocumentItem", back_populates="document", cascade="all, delete-orphan")


class PrintDocumentItem(Base):
    __tablename__ = "print_document_items"

    id = Column(Integer, primary_key=True)
    document_id = Column(Integer, ForeignKey("print_documents.id", ondelete="CASCADE"), nullable=False)
    item_name = Column(String, nullable=True)
    nomenclature_code = Column(String, nullable=True)
    unit_of_measure = Column(String, nullable=True)
    quantity = Column(Numeric(15, 4), nullable=True)
    ...
    document = relationship("PrintDocument", back_populates="items")
